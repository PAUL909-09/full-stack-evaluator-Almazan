...
//     // { to: "/evaluator/project-tasks", icon: FolderOpen, label: "Project Tasks" },
...
export default function Sidebar({ user, onLinkClick }) {
  ...
  // -------------------------------------------------
  // Detect if we are on a project-tasks page → show extra link
  // -------------------------------------------------
  const projectIdMatch = currentPath.match(/^\/evaluator\/project-tasks\/([^/]+)/);
  const projectId = projectIdMatch?.[1];

  const items = [...(baseMenu[user.role] ?? [])];

  // Insert “Assign Employees” **right after** Project List (only for Evaluator)
  if (user.role === "Evaluator" && projectId) {
    const insertIdx = items.findIndex((i) => i.to === "/evaluator/projects") + 1;
    items.splice(insertIdx, 0, {
      to: `/evaluator/assign-employees/${projectId}`,
      icon: UserPlus,
