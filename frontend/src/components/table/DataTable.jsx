...
// /**
//  * ✨ Modern Elegant Data Table (Blue Theme, No Scroll Bugs)
//  * Props:
//  * - title?: string
...
//       {/* 🧾 Scroll Wrapper Without Horizontal Issues */}
...
//                   // ✨ Modern hover animation (NO SCALE → no scrollbar)
//                   whileHover={{
//                     y: -3,
...
const DataTable = ({ title, columns, data, actions }) => {
  return (
    <motion.div
      ...
      transition={{ duration: 0.4 }}
      className="
        z-0  /* Added to prevent stacking above dropdown */
        overflow-visible 
        rounded-2xl 
        border border-[#A0DCFC]/40 
        bg-white 
    ...
    >
      ...
      <div className="overflow-x-visible overflow-y-visible">  {/* Changed from overflow-x-hidden to prevent clipping */}
        <table className="w-full border-collapse">
          <thead className="bg-[#F5FAFF]">
            <tr>
              {columns.map((col) => (
